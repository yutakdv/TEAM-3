...
public class SettingKeyHandler { // NOPMD
  ...
  public SettingKeyHandler(
      final SettingScreen screen, final InputManager inputManager, final Cooldown inputCooldown) {
    this.screen = screen;
  ...
  private void updateKeyIndexSelection() {
    ...
    if (inputManager.isKeyPressed(KeyEvent.VK_UP) && selectedKeyIndex > 0) {
      ...
      SoundManager.playeffect(SOUND_HOVER);
        ...
        && selectedKeyIndex < screen.getKeyItemsCount() - 1) {
      selectedKeyIndex++;
      SoundManager.playeffect(SOUND_HOVER);
    }
  ...
  private boolean isKeyDuplicate(final int newKey) {
    final int[] targetKeys =
        (screen.getSelectMenuItem() == MENU_P1_KEYS)
            ? screen.getPlayer1Keys()
            : screen.getPlayer2Keys();
    final int[] otherKeys =
        (screen.getSelectMenuItem() == MENU_P1_KEYS)
            ? screen.getPlayer2Keys()
            : screen.getPlayer1Keys();
