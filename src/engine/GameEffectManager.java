...
public class GameEffectManager {
    ...
    private void initializeStates() {
        for (int p = 0; p < NUM_PLAYERS; p++) {
            final Map<ItemEffectType, EffectState> map = new HashMap<>(); // NOPMD
            for (final ItemEffectType type : ItemEffectType.values()) {
                map.put(type, new EffectState()); // NOPMD
            }
            playerEffects.put(p, map);
        }
    }

    private EffectState getState(final int playerIndex, final ItemEffectType type) {
