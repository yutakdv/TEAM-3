...
/**
 * Manages the grid structure of enemy ships.
 * Handles storage, dimensions, and cleaning up of empty columns.
 */
...
public class EnemyShipGrid implements Iterable<EnemyShip> {
    ...
    public EnemyShipGrid(final int nShipsWide, final int nShipsHigh, final int startX, final int startY) {
    ...
    public void cleanUp() {
        ...
        for (final List<EnemyShip> column : this.enemyShips) {
            final int colHeight = column.get(column.size() - 1).getPositionY()
    ...
    // Getters
    public List<List<EnemyShip>> getColumns() { return enemyShips; }
